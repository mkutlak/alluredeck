...
Ready-to-use example scripts are available in the [examples/](examples/) directory. All scripts support optional authentication via environment variables.

- **Bash** (Linux/macOS/CI): [examples/send_results.sh](examples/send_results.sh)
...
# With auth
ALLURE_USERNAME=admin ALLURE_PASSWORD=secret ./send_results.sh --generate-report
```

- **PowerShell** (Windows/Azure DevOps): [examples/send_results.ps1](examples/send_results.ps1)
...
# With auth
$env:ALLURE_USERNAME="admin"; $env:ALLURE_PASSWORD="secret"; .\send_results.ps1 -GenerateReport
```
...
- **Jenkins Declarative Pipeline**: [examples/jenkins-pipeline.groovy](examples/jenkins-pipeline.groovy)

NOTE:

- Example result files used by the scripts: [examples/allure-results-example](examples/allure-results-example)
- Set `ALLURE_SERVER`, `ALLURE_PROJECT_ID`, and `ALLURE_RESULTS_DIR` environment variables to match your setup
...
##### Scripts
- Bash script: [examples/send_results.sh](examples/send_results.sh)
```sh
ALLURE_USERNAME=admin ALLURE_PASSWORD=secret ./send_results.sh
```

- PowerShell script: [examples/send_results.ps1](examples/send_results.ps1)
