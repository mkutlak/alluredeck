...
func (g *blockingGen) GenerateReport(_ context.Context, _ int64, _, _, _, _, _, _ string, _ bool, _, _ string) (string, error) {
	<-g.ch
	return "ok", nil
}
...
func (g *contextGen) GenerateReport(ctx context.Context, _ int64, _, _, _, _, _, _ string, _ bool, _, _ string) (string, error) {
	select {
	case <-g.ch:
		return "ok", nil
	case <-ctx.Done():
