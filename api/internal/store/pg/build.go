...
func (bs *BuildStore) NextBuildNumber(ctx context.Context, projectID int64) (int, error) {
	var maxOrder *int64
...
func (bs *BuildStore) InsertBuild(ctx context.Context, projectID int64, buildNumber int) error {
...
func (bs *BuildStore) UpdateBuildStats(ctx context.Context, projectID int64, buildNumber int, stats store.BuildStats) error {
...
func (bs *BuildStore) UpdateBuildCIMetadata(ctx context.Context, projectID int64, buildNumber int, ciMeta store.CIMetadata) error {
...
func (bs *BuildStore) GetBuildByNumber(ctx context.Context, projectID int64, buildNumber int) (store.Build, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return store.Build{}, fmt.Errorf("%w: project=%d build=%d", store.ErrBuildNotFound, projectID, buildNumber)
		}
...
func (bs *BuildStore) GetPreviousBuild(ctx context.Context, projectID int64, buildNumber int) (store.Build, error) {
	row := bs.pool.QueryRow(ctx, buildSelectCols+`
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return store.Build{}, fmt.Errorf("%w: no previous build for project=%d build=%d", store.ErrBuildNotFound, projectID, buildNumber)
		}
...
func (bs *BuildStore) GetLatestBuild(ctx context.Context, projectID int64) (store.Build, error) {
	row := bs.pool.QueryRow(ctx, buildSelectCols+`
		WHERE project_id=$1 AND is_latest=TRUE`, projectID)
	b, err := scanBuildRow(row)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return store.Build{}, fmt.Errorf("%w: no latest build for project=%d", store.ErrBuildNotFound, projectID)
		}
...
func (bs *BuildStore) ListBuilds(ctx context.Context, projectID int64) ([]store.Build, error) {
...
func (bs *BuildStore) ListBuildsPaginated(ctx context.Context, projectID int64, page, perPage int) ([]store.Build, int, error) {
...
func (bs *BuildStore) PruneBuilds(ctx context.Context, projectID int64, keep int) ([]int, error) {
...
func (bs *BuildStore) SetLatest(ctx context.Context, projectID int64, buildNumber int) error {
...
func (bs *BuildStore) DeleteAllBuilds(ctx context.Context, projectID int64) error {
	_, err := bs.pool.Exec(ctx, "DELETE FROM builds WHERE project_id=$1", projectID)
	if err != nil {
		return fmt.Errorf("delete all builds for project %d: %w", projectID, err)
	}
	return nil
}
...
func (bs *BuildStore) GetDashboardData(ctx context.Context, sparklineDepth int) ([]store.DashboardProject, error) {
	query := `
		SELECT DISTINCT ON (p.id)
		    p.id, p.slug, p.parent_id, p.display_name, p.report_type, p.created_at,
	...
	projectMap := make(map[int64]*store.DashboardProject)
	var orderedIDs []int64

	for rows.Next() {
		var projID int64
		var projSlug string
		var projParentID *int64
		var projDisplayName string
		var projReportType string
		var projCreatedAt time.Time
		// Nullable build fields (LEFT JOIN may produce NULLs).
		var buildID *int64
		var buildProjID *int64
		...
		if err := rows.Scan(
			&projID, &projSlug, &projParentID, &projDisplayName, &projReportType, &projCreatedAt,
...
func (bs *BuildStore) DeleteBuild(ctx context.Context, projectID int64, buildNumber int) error {
	_, err := bs.pool.Exec(ctx,
...
func (bs *BuildStore) UpdateBuildBranchID(ctx context.Context, projectID int64, buildNumber int, branchID int64) error {
	tag, err := bs.pool.Exec(ctx,
...
func (bs *BuildStore) SetLatestBranch(ctx context.Context, projectID int64, buildNumber int, branchID *int64) error {
	if branchID == nil {
...
func (bs *BuildStore) PruneBuildsBranch(ctx context.Context, projectID int64, keep int, branchID *int64) ([]int, error) {
	if keep <= 0 {
		return nil, nil
	}
...
// Returns the build_orders of removed builds.
func (bs *BuildStore) PruneBuildsByAge(ctx context.Context, projectID int64, olderThan time.Time) ([]int, error) {
	rows, err := bs.pool.Query(ctx, `
		SELECT build_order FROM builds
...
func (bs *BuildStore) ListBuildsPaginatedBranch(ctx context.Context, projectID int64, page, perPage int, branchID *int64) ([]store.Build, int, error) {
	var totalCount int
	var err error
	if branchID != nil {
		err = bs.pool.QueryRow(ctx,
...
func (bs *BuildStore) ListBuildsInRange(ctx context.Context, projectID int64, branchID *int64, from, to time.Time, limit int) ([]store.Build, int, error) {
	var totalCount int
	var err error
	if branchID != nil {
		err = bs.pool.QueryRow(ctx,
...
func (bs *BuildStore) ExistingBuildNumbers(ctx context.Context, projectID int64) (map[int]struct{}, error) {
	rows, err := bs.pool.Query(ctx,
		"SELECT build_order FROM builds WHERE project_id=$1", projectID)
	if err != nil {
		return nil, fmt.Errorf("existing build orders for %d: %w", projectID, err)
	}
	defer rows.Close()
...
func (bs *BuildStore) InsertMissingBuilds(ctx context.Context, projectID int64, missing []int) error {
	if len(missing) == 0 {
		return nil
	}
	tx, err := bs.pool.Begin(ctx)
	...
	for _, bo := range missing {
		if _, err := tx.Exec(ctx,
			...
			return fmt.Errorf("insert build %d for project %d: %w", bo, projectID, err)
		}
	}
	if err := tx.Commit(ctx); err != nil {
...
func (bs *BuildStore) BuildsWithMissingStats(ctx context.Context, projectID int64) ([]int, error) {
	rows, err := bs.pool.Query(ctx,
		"SELECT build_order FROM builds WHERE project_id=$1 AND stat_total IS NULL", projectID)
	if err != nil {
		return nil, fmt.Errorf("builds with missing stats for %d: %w", projectID, err)
	}
	defer rows.Close()
...
// The slug parameter is used for storage lookups (filesystem/S3 paths); projectID is the DB primary key.
func (bs *BuildStore) BatchSyncStats(ctx context.Context, projectID int64, slug string, buildNumbers []int, st storage.Store) error {
