...
func TestIPRateLimiter_Allow(t *testing.T) {
	rl := NewIPRateLimiter(2, 2, 5*time.Minute) // 2 req/s, burst 2
...
func TestIPRateLimiter_IPIsolation(t *testing.T) {
	rl := NewIPRateLimiter(1, 1, 5*time.Minute) // 1 req/s, burst 1
...
func TestIPRateLimiter_Refill(t *testing.T) {
	rl := NewIPRateLimiter(1000, 1, 5*time.Minute) // high rate so refill is fast
...
func TestIPRateLimiter_Cleanup(t *testing.T) {
	rl := NewIPRateLimiter(10, 10, 10*time.Millisecond) // very short TTL
...
func TestRateLimitMiddleware_NormalTraffic(t *testing.T) {
	rl := NewIPRateLimiter(10, 10, 5*time.Minute)
...
func TestRateLimitMiddleware_ExcessiveRequests(t *testing.T) {
	rl := NewIPRateLimiter(1, 2, 5*time.Minute) // burst 2
...
func TestRateLimitMiddleware_XForwardedFor(t *testing.T) {
	rl := NewIPRateLimiter(1, 1, 5*time.Minute)
	...
	if rr2.Code != http.StatusTooManyRequests {
		t.Errorf("expected 429 for same X-Forwarded-For IP, got %d", rr2.Code)
	}
	...
	if rr3.Code != http.StatusOK {
		t.Errorf("expected 200 for different forwarded IP, got %d", rr3.Code)
	}
}

func TestStartCleanup_StopsOnDone(t *testing.T) {
	rl := NewIPRateLimiter(10, 10, 10*time.Millisecond)
	rl.Allow("10.0.0.1")
