...
func (m *riverMockGen) GenerateReport(_ context.Context, _ int64, _, _, _, _, _ string, _ bool, _, _ string) (string, error) {
	return m.out, nil
}
...
func newTestRiverJM(t *testing.T, s *pg.PGStore, gen runner.ReportGenerator) *runner.RiverJobManager {
	...
	jm, err := runner.NewRiverJobManager(s.Pool(), gen, nil, webhookStore, buildStore, nil, "", 1, 5*time.Minute, logger)
	if err != nil {
