...
function getMimeBadge(mimeType: string, name: string): { label: string; className: string } {
  if (mimeType.startsWith('image/'))
    return { label: 'IMAGE', className: 'bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-300' }
  if (mimeType.startsWith('text/') || mimeType === 'application/json' || mimeType === 'application/xml')
    return { label: 'LOG', className: 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' }
  if (isPlaywrightTrace(name, mimeType))
    return { label: 'TRACE', className: 'bg-violet-100 text-violet-700 dark:bg-violet-900 dark:text-violet-300' }
  if (mimeType.startsWith('video/'))
    return { label: 'VIDEO', className: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300' }
  return { label: 'OTHER', className: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }
}

export function AttachmentRow({ attachment, onView }: AttachmentRowProps) {
  const { mime_type, name, size_bytes, url } = attachment
  const isImage = mime_type.startsWith('image/')
  const isText = mime_type.startsWith('text/') || mime_type === 'application/json' || mime_type === 'application/xml'
